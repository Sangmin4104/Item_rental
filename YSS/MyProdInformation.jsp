...
<body>
    ...
    %> 
        ...
        <div>
            <span class="title"><strong>상 세 정 보</strong></span>
        </div>
        <hr class="line1">
        <hr class="line2">
        <hr class="line3">
...
<div class="card">
    ...
    <button type="submit" class="buttonl" onclick="location='Product_RegistarChangeForm.jsp?P_PNUM=<%=P_PNUM%>&P_PNAME=<%=P_PNAME%>&DNAME=<%=DNAME%>&SNAME=<%=SNAME%>&P_SNUM=<%=P_SNUM%>&P_AMOUNT=<%=P_AMOUNT%>&P_RENTAL=<%=P_RENTAL%>&P_IMAGES=<%=P_IMAGES%>'">수정</button> 

    <button type="submit" class="buttonl2" onclick="test()">삭제</button>
    <script>
      function test() {
          if (confirm("물픔을 삭제하시겠습니까?")) {
              location.href="Product_RegistarDeletePro.jsp?P_PNUM=<%=P_PNUM%>&P_PNAME=<%=P_PNAME%>&DNAME=<%=DNAME%>&SNAME=<%=SNAME%>&P_SNUM=<%=P_SNUM%>&P_AMOUNT=<%=P_AMOUNT%>&P_RENTAL=<%=P_RENTAL%>";
          } else {
              alert("취소하셨습니다.");
              location.href="MyProdInformation.jsp?P_PNUM=<%=P_PNUM%>&P_PNAME=<%=P_PNAME%>&DNAME=<%=DNAME%>&SNAME=<%=SNAME%>&P_SNUM=<%=P_SNUM%>&P_AMOUNT=<%=P_AMOUNT%>&P_RENTAL=<%=P_RENTAL%>";
          }
      }
  </script>
  </div>
