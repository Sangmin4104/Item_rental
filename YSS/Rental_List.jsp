...
String sql = "";
 try{
  ...
  conn=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      sql = "select i.dname, i.snum, i.sname, to_char(pd.pr_start, 'YYYY-MM-DD') as pr_start, to_char(pd.pr_end, 'YYYY-MM-DD') as pr_end from prod p, id i, prod_doc pd where p.p_pnum = pd.pr_pnum and p.p_snum = i.snum and pd.pr_yid =? and p.p_snum = ?";
      pstmt = conn.prepareStatement(sql); 
      pstmt.setString(1, PR_YID);
      pstmt.setString(2, P_SNUM);
...
<div class="card">
  <h1 style="text-align:center; position:relative; top:45px; z-index:1;">대 여 증</h1>
    <div>
      ...
      <span style="position:relative; top:110px; left:220px; z-index:1;">상기 물품을 <strong><%=pr_start%> </strong> 에 대여하였음.</span><br>
      <span style="position:relative; top:120px; left:220px; z-index:1;"><strong><%=pr_end%></strong> 까지 <Strong><%=DNAMEE%> 학과사무실</Strong>로 반납바람.</span>
      <span style="position:relative; top:240px; left:200px; z-index:1;">관리자 : <%=DNAMEE%> <%=SNAMEE%></span><br>
