...
<body>
    ...
    %> 
        ...
        <div>
            <span class="title"><strong>등 록 물 품 확 인</strong></span>
        </div>
        <i style="font-size:25px; position:relative; top:85px; left: 1150px; cursor: pointer;" class="fa-solid fa-trash-can" Onclick="location='Product_RegistarTrash.jsp'"></i>
        <hr class="line">
        <%
...
int total = 0;
 ...
 try{
  ...
  conn1=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
    ...
    String sqlCount = "SELECT count(*) FROM ID i, PROD p where i.snum = p.p_snum and p.p_snum = ? and p.p_state='O' ORDER BY P_PNUM";
...
String sql = "";
 try{
  ...
  conn=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      sql = "SELECT p.p_pnum, p.p_pname, i.dname, i.sname, p.p_amount, p.p_rental, p.p_snum, p.p_images FROM ID i, PROD p WHERE p.p_snum = i.snum and i.snum = ? and p.p_state = 'O' ORDER BY p.p_pnum";
...
%>
<div class="card" onclick="location='MyProdInformation.jsp?P_PNUM=<%=P_PNUM%>&P_PNAME=<%=P_PNAME%>&DNAME=<%=DNAME%>&SNAME=<%=SNAME%>&P_SNUM=<%=P_SNUM%>&P_AMOUNT=<%=P_AMOUNT%>&P_RENTAL=<%=P_RENTAL%>&P_IMAGES=<%=P_IMAGES%>'">
    <div class="img">
