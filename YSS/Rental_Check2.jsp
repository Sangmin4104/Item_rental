...
<body>
    ...
    %> 
        ...
        <div>
            <span class="title"><strong>물 품 반 납</strong></span>
        </div>
...
String sql = "";
 try{
  ...
  conn=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      sql = "SELECT pr.pr_cancel, p.p_amount, pr.pr_yid, pr.pr_pnum, pr.pr_snum, i.dname, i.sname, pr.pr_amount, to_char(pr.pr_start, 'YYYY-MM-DD') pr_start, to_char(pr.pr_end, 'YYYY-MM-DD') pr_end,p.p_pname FROM prod p, ID i, PROD_DOC pr WHERE i.snum = ? and pr.pr_yid = ? and pr.pr_snum = i.snum and p.p_pnum = pr.pr_pnum";
      pstmt = conn.prepareStatement(sql); 
      pstmt.setString(1, SNUM);
      pstmt.setString(2, PR_YID);
      myRS = pstmt.executeQuery();
      if(myRS != null){
        while(myRS.next()){
          String PPR_CANCEL = myRS.getString("PR_CANCEL");
          int PPR_YID = myRS.getInt("PR_YID");  
          int PPR_PNUM = myRS.getInt("PR_PNUM");
          String PPR_SNUM = myRS.getString("PR_SNUM");
          String PP_PNAME = myRS.getString("P_PNAME");
          String DDNAME = myRS.getString("DNAME");
          String SSNAME = myRS.getString("SNAME");
          int PPR_AMOUNT = myRS.getInt("PR_AMOUNT");
          int PP_AMOUNT = myRS.getInt("P_AMOUNT");
          String PPR_START = myRS.getString("PR_START");
          String PPR_END = myRS.getString("PR_END");

%>

<tr align="center">
   <form method='post' action='Rental_Check_to.jsp' >
    <td ><input type="text"  style=" text-align:center; border:none;" name="PR_CANCEL" value= '<%=PPR_CANCEL%>'readonly></td>
    <td ><input type="text"  style="text-align:center; border:none;"name="PR_YID" value= '<%=PPR_YID%>'readonly></td>
    <td hidden><input type="text"  style="border:none;" name="PR_PNUM" value= '<%=PPR_PNUM%>'readonly></td>
    <td hidden><input type="text" style="border:none;" name="PR_SNUM" value= '<%=PPR_SNUM%>'readonly></td>
    <td ><input type="text" style="text-align:center; border:none;" name="P_PNAME" value= '<%=PP_PNAME%>'readonly></td>
    <td hidden><input type="text" style="text-align:center; border:none;" name="DNAME" value= '<%=DDNAME%>'readonly></td>
    <td hidden><input type="text"  style="text-align:center; border:none;"name="SNAME" value= '<%=SSNAME%>'readonly></td>
    <td ><input type="text"  style="text-align:center; border:none;"name="PR_AMOUNT" value= '<%=PPR_AMOUNT%>'readonly></td>
    <td ><input type="text"  style="text-align:center; border:none;"name="PR_START" value= '<%=PPR_START%>'readonly></td>
    <td ><input type="text"  style="text-align:center; border:none;"name="PR_END" value= '<%=PPR_END%>'readonly></td>
    <td hidden><input type="text"  style="text-align:center; border:none;"id="qq" value= '<%=PP_AMOUNT%>'  readonly></td>
    <td hidden><input type="text" style="text-align:center; border:none;" id="qqq" value= '<%=PPR_AMOUNT%>'  readonly></td>
    <td hidden><input type="text"  style="text-align:center; border:none;"id="w"  name="w" readonly></td>
    
    <td>
    <%
        if(PR_CANCEL.equals("대여중")){
%>
    <button type="submit" class="buttonl" style=" position:relative;">반납</button> 
<%}
%>
    </td>
    </form>
