...
<body style="position:relative; top:-37px;">
    <div class="container">
        <hr class="line">
        <div class="squ">
...
int total = 0;
 ...
 try{
  ...
  conn1=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      String sqlCount = "SELECT count(*) FROM ID i, PROD p where i.snum = p.p_snum and i.dname = ? and p.p_amount > 0 and p.p_state='O' ORDER BY P_PNUM";
      ...
      <%}
      else if(SHEAD==1 && total > 1){
%>

<strong class="pcount" >총 <%=total%>개</strong>
<%
}else{
      String sqlCount1 = "SELECT count(*) FROM ID i, PROD p where i.snum = p.p_snum and i.dname = ? and p.p_amount > 0 and i.shead=0 and p.p_state='O' ORDER BY P_PNUM";
...
String sql = "";
 try{
  ...
  conn=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      sql = "SELECT p.p_pnum, p.p_pname, i.dname, i.sname, p.p_amount, p.p_rental, p.p_snum, p.p_images FROM ID i, PROD p where p.p_amount > 0 and i.snum = p.p_snum and i.dname = ? AND p.p_state='O'ORDER BY P_PNUM";
      pstmt = conn.prepareStatement(sql); 
...
<%
  ...
  sql = "SELECT p.p_pnum, p.p_pname, i.dname, i.sname, p.p_amount, p.p_rental, p.p_snum, p.p_images FROM ID i, PROD p where p.p_amount > 0 and i.snum = p.p_snum and i.dname = ? AND i.shead=0 AND p.p_state='O'ORDER BY P_PNUM";
  pstmt = conn.prepareStatement(sql); 
  pstmt.setString(1,department);
  myRS = pstmt.executeQuery();
  if(myRS != null){
