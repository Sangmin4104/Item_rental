...
<body>
    <div class="container">
        ...
        <div class="squ4">
            ...
            <form method="post" name="department" action="MainPageSearchDepartmentList.jsp">
...
int total = 0;
 ...
 try{
  ...
  conn1=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      String sqlCount = "SELECT count(*) FROM ID i, PROD p where p.p_amount > 0 and i.snum = p.p_snum and p.p_pname like ? and p.p_state='O' ORDER BY P_PNUM";
      ...
      if(total == 0){
        %>
        <strong class="pcount" >물품이 없습니다.</strong>
        <%}
      else if(SHEAD == 1){
...
<%
  ...
  String sqlCount1 = "SELECT count(*) FROM ID i, PROD p where p.p_amount > 0 and i.snum = p.p_snum and p.p_pname like ? and i.shead=0 and p.p_state='O' ORDER BY P_PNUM";
...
String sql = "";
 try{
  ...
  conn=DriverManager.getConnection(jdbcUrl, dbId, dbPass );
      ...
      sql = "SELECT p.p_pnum, p.p_pname, i.dname, i.sname, p.p_amount, p.p_rental, p.p_snum, p.p_images FROM ID i, PROD p where p.p_amount > 0 and p.p_state='O' and i.snum = p.p_snum and p.p_pname like ? ORDER BY P_PNUM";
      pstmt = conn.prepareStatement(sql); 
...
<div class="card" onclick="location='Information.jsp?P_PNUM=<%=P_PNUM%>&P_PNAME=<%=P_PNAME%>&DNAME=<%=DNAME%>&SNAME=<%=SNAME%>&P_SNUM=<%=P_SNUM%>&P_AMOUNT=<%=P_AMOUNT%>&P_RENTAL=<%=P_RENTAL%>'">
    ...
    sql = "SELECT p.p_pnum, p.p_pname, i.dname, i.sname, p.p_amount, p.p_rental, p.p_snum, p.p_images FROM ID i, PROD p where p.p_amount > 0 and p.p_state='O' and i.snum = p.p_snum and i.shead=0 and p.p_pname like ? ORDER BY P_PNUM";
  pstmt = conn.prepareStatement(sql); 
  pstmt.setString(1,"%" + keyWord + "%");
  myRS = pstmt.executeQuery();
  if(myRS != null){
